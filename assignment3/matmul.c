...
Currently supports matmul with dimensions being even sizes
...
int main(int argc, char** argv){
    ...
    if (row_a % 2 != 0 || col_a % 2 != 0 || row_b % 2 != 0 || col_b % 2 != 0) {
        fprintf(stderr, "Currently supports only even dimensions.\n");
        exit(1);
    }
    if(col_a != row_b){
    ...
    int subSize = (row_a / 2) * (col_b / 2);
    int *subRes = (int*)malloc(sizeof(int) * subSize);
    ...
    // Perform submatrix multiplications
    if(rank == 0){
        for(int i = 0 ; i < (row_a / 2); i++){
            for(int j = 0; j < (col_b / 2); j++){
                for(int k = 0 ; k < col_a; k++){
    ...
    if(rank == 1){
        for(int i = 0 ; i < (row_a / 2); i++){
            for(int j = 0; j < (col_b / 2); j++){
                for(int k = 0 ; k < col_a; k++){
                    subRes[i * (col_b / 2) + j] += matrixA[i * col_a + k] * matrixB[k * col_b + (j + col_b / 2)];
                }
            }
        }
    }
    ...
    if(rank == 3){
        for(int i = 0 ; i < (row_a / 2); i++){
            for(int j = 0; j < (col_b / 2); j++){
                for(int k = 0 ; k < col_a; k++){
                    subRes[i * (col_b / 2) + j] += matrixA[(i + row_a / 2) * col_a + k] * matrixB[k * col_b + (j + col_b / 2)];
                }
            }
        }
    }
    ...
    MPI_Gather(subRes, subSize, MPI_INT, resultMatrix, subSize, MPI_INT, 0, MPI_COMM_WORLD);
    int *originalResult;
    if(rank == 0){
        int index = 0;
        for(int i = 0; i < row_a / 2; i++){
